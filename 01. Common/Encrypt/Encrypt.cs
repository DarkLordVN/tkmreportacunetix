...
namespace TKM.Utils
{
    public class Encrypt
    {
        ...
        private const string KEY = "NotifierEntity-Co-Dong-WebApp-Key";
        ...
        public static bool CheckLicense(string keyLicense)
        {
            var check = false;
            //var licenseInfi1 = Encrypt.EncryptAndHash("tkm20217");
            var licenseInfi = Encrypt.EncryptAndHash("tkminfinity");
            if (keyLicense.Equals(licenseInfi))
            {
                check = true;
            }
            var deLicense = Encrypt.DecryptWithHash(keyLicense);

            var timeLicense = 0;
            var currentTime = 0;
            int.TryParse(deLicense.Replace("tkm",""), out timeLicense);
            int.TryParse(DateTime.Now.Year + "" + DateTime.Now.Month, out currentTime);
            if(timeLicense >= currentTime)
            {
                check = true;
            }
            return check;
    ...
    public sealed class CRYPTION
    {
        ...
        public string DECRYPT(string s)// chua dung voi moi truong hop.
        {
            //new instance of algorithm creation
            Algorithm = new RijndaelManaged();
            ...
            string Pass = strReader.ReadToEnd().ToString(CultureInfo.InvariantCulture);
            return Pass;
            //.ReadToEnd();
        }
    }
    ...
    public sealed class UnixCrypt
    {
        ...
        public static string Crypt(string textToEncrypt)// lay key ngau nhien
        {
