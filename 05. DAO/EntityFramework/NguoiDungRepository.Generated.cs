...
{   
	...
	{
		private IRepository<NguoiDung> _repository {get;set;}
		public IRepository<NguoiDung> Repository
		{
			get { return _repository; }
			set { _repository = value; }
		}
		
		public NguoiDungRepository(IRepository<NguoiDung> repository, IUnitOfWork unitOfWork)
    	{
    		Repository = repository;
			Repository.UnitOfWork = new EFUnitOfWork(); 
    	}

		public List<NguoiDung> GetList(Expression<Func<NguoiDung, bool>> where)
        {
            try
            {
                IEnumerable<NguoiDung> lResult;
                if (where != null)
                {
		...
		public List<NguoiDung> GetList(Expression<Func<NguoiDung, bool>> where, int pageIndex, int pageSize, ref int totalItem, Func<NguoiDung, object> orderCol = null, bool isDecending = true)
        {
            try
            {
                IEnumerable<NguoiDung> lResult;
        ...
        public NguoiDung GetByFilter(Expression<Func<NguoiDung, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool CheckByFilter(Expression<Func<NguoiDung, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool Add(List<NguoiDung> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
		...
		public bool Update(List<NguoiDung> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
