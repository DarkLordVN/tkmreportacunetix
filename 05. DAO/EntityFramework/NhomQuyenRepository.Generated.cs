...
{   
	...
	{
		private IRepository<NhomQuyen> _repository {get;set;}
		public IRepository<NhomQuyen> Repository
		{
			get { return _repository; }
			set { _repository = value; }
		}
		
		public NhomQuyenRepository(IRepository<NhomQuyen> repository, IUnitOfWork unitOfWork)
    	{
    		Repository = repository;
			Repository.UnitOfWork = new EFUnitOfWork(); 
    	}

		public List<NhomQuyen> GetList(Expression<Func<NhomQuyen, bool>> where)
        {
            try
            {
                IEnumerable<NhomQuyen> lResult;
                if (where != null)
                {
		...
		public List<NhomQuyen> GetList(Expression<Func<NhomQuyen, bool>> where, int pageIndex, int pageSize, ref int totalItem, Func<NhomQuyen, object> orderCol = null, bool isDecending = true)
        {
            try
            {
                IEnumerable<NhomQuyen> lResult;
		...
		public NhomQuyen GetById(int id)
        {
            try
            {
        ...
        public NhomQuyen GetByFilter(Expression<Func<NhomQuyen, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool CheckByFilter(Expression<Func<NhomQuyen, bool>> where)
        {
            try
            {
                Repository.Save();
        ...
        public bool Add(NhomQuyen entity)
        {
		...
		public bool Add(List<NhomQuyen> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
        ...
        public bool Update(NhomQuyen entity)
        {
            try
            {
		...
		public bool Update(List<NhomQuyen> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
