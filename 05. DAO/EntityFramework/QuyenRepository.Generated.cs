...
{   
	...
	{
		private IRepository<Quyen> _repository {get;set;}
		public IRepository<Quyen> Repository
		{
			get { return _repository; }
			set { _repository = value; }
		}
		
		public QuyenRepository(IRepository<Quyen> repository, IUnitOfWork unitOfWork)
    	{
    		Repository = repository;
			Repository.UnitOfWork = new EFUnitOfWork(); 
    	}

		public List<Quyen> GetList(Expression<Func<Quyen, bool>> where)
        {
            try
            {
                IEnumerable<Quyen> lResult;
                if (where != null)
                {
		...
		public List<Quyen> GetList(Expression<Func<Quyen, bool>> where, int pageIndex, int pageSize, ref int totalItem, Func<Quyen, object> orderCol = null, bool isDecending = true)
        {
            try
            {
                IEnumerable<Quyen> lResult;
        ...
        public Quyen GetByFilter(Expression<Func<Quyen, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool CheckByFilter(Expression<Func<Quyen, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool Add(List<Quyen> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
        ...
        public bool Update(Quyen entity)
        {
            try
            {
		...
		public bool Update(List<Quyen> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
