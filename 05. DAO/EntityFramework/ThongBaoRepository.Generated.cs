...
{   
	...
	{
		private IRepository<ThongBao> _repository {get;set;}
		public IRepository<ThongBao> Repository
		{
			get { return _repository; }
			set { _repository = value; }
		}
		
		public ThongBaoRepository(IRepository<ThongBao> repository, IUnitOfWork unitOfWork)
    	{
    		Repository = repository;
			Repository.UnitOfWork = new EFUnitOfWork(); 
    	}

		public List<ThongBao> GetList(Expression<Func<ThongBao, bool>> where)
        {
            try
            {
                IEnumerable<ThongBao> lResult;
                if (where != null)
                {
		...
		public List<ThongBao> GetList(Expression<Func<ThongBao, bool>> where, int pageIndex, int pageSize, ref int totalItem, Func<ThongBao, object> orderCol = null, bool isDecending = true)
        {
            try
            {
                IEnumerable<ThongBao> lResult;
        ...
        public ThongBao GetByFilter(Expression<Func<ThongBao, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool CheckByFilter(Expression<Func<ThongBao, bool>> where)
        {
            try
            {
                Repository.Save();
        ...
        public bool Add(ThongBao entity)
        {
		...
		public bool Add(List<ThongBao> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
		...
		public bool Update(List<ThongBao> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
