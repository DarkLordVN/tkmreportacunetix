...
{   
	...
	{
		private IRepository<WebsiteScan> _repository {get;set;}
		public IRepository<WebsiteScan> Repository
		{
			get { return _repository; }
			set { _repository = value; }
		}
		
		public WebsiteScanRepository(IRepository<WebsiteScan> repository, IUnitOfWork unitOfWork)
    	{
    		Repository = repository;
			Repository.UnitOfWork = new EFUnitOfWork(); 
    	}

		public List<WebsiteScan> GetList(Expression<Func<WebsiteScan, bool>> where)
        {
            try
            {
                IEnumerable<WebsiteScan> lResult;
                if (where != null)
                {
		...
		public List<WebsiteScan> GetList(Expression<Func<WebsiteScan, bool>> where, int pageIndex, int pageSize, ref int totalItem, Func<WebsiteScan, object> orderCol = null, bool isDecending = true)
        {
            try
            {
                IEnumerable<WebsiteScan> lResult;
		...
		public WebsiteScan GetById(int id)
        {
            try
            {
        ...
        public WebsiteScan GetByFilter(Expression<Func<WebsiteScan, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool CheckByFilter(Expression<Func<WebsiteScan, bool>> where)
        {
            try
            {
                Repository.Save();
        ...
        public bool Add(WebsiteScan entity)
        {
		...
		public bool Add(List<WebsiteScan> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
        ...
        public bool Update(WebsiteScan entity)
        {
            try
            {
		...
		public bool Update(List<WebsiteScan> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
