...
{   
	...
	{
		private IRepository<v_Website> _repository {get;set;}
		public IRepository<v_Website> Repository
		{
			get { return _repository; }
			set { _repository = value; }
		}
		
		public v_WebsiteRepository(IRepository<v_Website> repository, IUnitOfWork unitOfWork)
    	{
    		Repository = repository;
			Repository.UnitOfWork = new EFUnitOfWork(); 
    	}

		public List<v_Website> GetList(Expression<Func<v_Website, bool>> where)
        {
            try
            {
		...
		public List<v_Website> GetList(Expression<Func<v_Website, bool>> where, int pageIndex, int pageSize, ref int totalItem, Func<v_Website, object> orderCol = null, bool isDecending = true)
        {
            try
            {
                IEnumerable<v_Website> lResult;
		...
		public v_Website GetById(int id)
        {
            try
            {
            ...
            {
                OutputLog.WriteOutputLog(ex);
                return new v_Website();
            }
        }
        public v_Website GetByFilter(Expression<Func<v_Website, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool CheckByFilter(Expression<Func<v_Website, bool>> where)
        {
            try
            {
                Repository.Save();
        ...
        public bool Add(v_Website entity)
        {
		...
		public bool Add(List<v_Website> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
        ...
        public bool Update(v_Website entity)
        {
            try
            {
		...
		public bool Update(List<v_Website> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
